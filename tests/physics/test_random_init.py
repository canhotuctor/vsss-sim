...
class TestNumpyResetRandom:
    ...
    def test_blue_in_left_half(self):
        for seed in range(10):
            s = self._reset(seed)
            xs = s.robots[config.TEAM_BLUE, :, 0]
            assert np.all(xs < 0), f"seed={seed}: blue robot in right half: {xs}"
            assert np.all(xs >= -_HALF_L + _MARGIN)

    def test_yellow_in_right_half(self):
        for seed in range(10):
            s = self._reset(seed)
            xs = s.robots[config.TEAM_YELLOW, :, 0]
            assert np.all(xs > 0), f"seed={seed}: yellow robot in left half: {xs}"
            assert np.all(xs <= _HALF_L - _MARGIN)
...
class TestJaxResetRandom:
    ...
    def test_blue_in_left_half(self):
        for seed in range(10):
            s = self._reset(seed)
            xs = s.robots[config.TEAM_BLUE, :, 0]
            assert jnp.all(xs < 0), f"seed={seed}: blue robot in right half"
    ...
    def test_yellow_in_right_half(self):
        for seed in range(10):
            s = self._reset(seed)
            xs = s.robots[config.TEAM_YELLOW, :, 0]
            assert jnp.all(xs > 0), f"seed={seed}: yellow robot in left half"
            assert jnp.all(xs <= _HALF_L - _MARGIN)
    ...
    def test_vmap_compatible(self):
        ...
        # All blue xs negative, all yellow xs positive
        assert jnp.all(states.robots[:, config.TEAM_BLUE, :, 0] < 0)
        assert jnp.all(states.robots[:, config.TEAM_YELLOW, :, 0] > 0)
...
class TestVSSVecEnvRandomInit:
    ...
    def test_blue_starts_left(self):
        env = VSSVecEnv(num_envs=32, init_mode=InitMode.RANDOM)
        env.reset(seed=0)
        blue_x = np.array(env._state.robots[:, config.TEAM_BLUE, :, 0])
        assert np.all(blue_x < 0)
        env.close()

    def test_yellow_starts_right(self):
        env = VSSVecEnv(num_envs=32, init_mode=InitMode.RANDOM)
        env.reset(seed=0)
        yellow_x = np.array(env._state.robots[:, config.TEAM_YELLOW, :, 0])
        assert np.all(yellow_x > 0)
        env.close()
